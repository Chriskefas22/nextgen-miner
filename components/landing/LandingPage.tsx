import Image from 'next/image'; import Link from 'next/link'; import { createClient } from '@/lib/supabase/server'; import NetworkCore from '@/components/landing/NetworkCore';
type BonusStatus = { total_slots?: number; claimed_slots?: number; remaining_slots?: number; bonus_miner?: { name?: string; hashrate?: number } | null; };
const miners = [ { name: 'Basic CPU', price: '500', rate: '20 H/s', level: 'Lv. 1–10', image: '/assets/miners/basic-cpu.webp' }, { name: 'Entry GPU', price: '1,000', rate: '60 H/s', level: 'Lv. 1–10', image: '/assets/miners/entry-gpu.webp' }, { name: 'Mini Rig', price: '2,500', rate: '150 H/s', level: 'Lv. 1–10', image: '/assets/miners/mini-rig.webp' }, { name: 'Gaming PC', price: '7,500', rate: '450 H/s', level: 'Lv. 1–10', image: '/assets/miners/gaming-pc.webp' }, { name: 'Performance Rig', price: '15,000', rate: '850 H/s', level: 'Lv. 1–10', image: '/assets/miners/performance-rig.webp' }, ];
export default async function LandingPage() { const supabase = await createClient(); const { data } = await supabase.rpc('nextgen_get_registration_bonus_status'); const bonus = (data ?? null) as BonusStatus | null;
const total = typeof bonus?.total_slots === 'number' ? bonus.total_slots : 0; const claimed = typeof bonus?.claimed_slots === 'number' ? bonus.claimed_slots : 0; const remaining = typeof bonus?.remaining_slots === 'number' ? bonus.remaining_slots : Math.max(total - claimed, 0); const progress = total > 0 ? Math.min(100, Math.max(0, (claimed / total) * 100)) : 0; const fmt = (value: unknown) => typeof value === 'number' ? value.toLocaleString('en-US') : '—'; const bonusMinerName = bonus?.bonus_miner?.name || 'Entry GPU'; const bonusHashrate = typeof bonus?.bonus_miner?.hashrate === 'number' ? ${bonus.bonus_miner.hashrate.toLocaleString('en-US')} H/s : '60 H/s';
return (    N NEXTGEN MINER 
<nav className="navlinks" aria-label="Primary navigation">
    ...
    <div className="hero-content">
      ...
      <aside className="hash-panel" aria-label="Network status">
        <div className="panel-top">
          ...
          <span className="panel-live"><i aria-hidden="true" />ONLINE</span>
        </div>

        <div className="core-visual">
          <div className="network-globe-wrap"><NetworkCore /></div>
        ...
        <div className="network-chip-grid" aria-label="Network information">
          <div><small>GLOBAL HASHRATE</small><b>LIVE</b><em>Connected</em></div>
          <div><small>MINER ENGINE</small><b>READY</b><em>Server-side</em></div>
          <div><small>NETWORK NODES</small><b>ONLINE</b><em>Protected</em></div>
          <div><small>UPTIME</small><b>ACTIVE</b><em>Monitoring</em></div>
        </div>
      </aside>
    </div>
  ...
  <section className="campaign" aria-labelledby="campaign-title">
    <div className="campaign-copy">
      ...
      <p>
        Complete registration and verification to become eligible for the launch allocation.
        Availability is controlled by the database and remaining campaign slots.
      </p>
      ...
      <div className="allocation-bar">
        <div className="allocation-track">
          <i style={{ width: `${Math.max(0, Math.min(100, 100 - progress))}%` }} />
        </div>
    ...
    <div className="bonus-visual" aria-label={`${bonusMinerName} registration bonus visual`}>
      ...
      <div className="gpu-platform">
        ...
        <Image
          src="/assets/miners/entry-gpu.webp"
          alt=""
          fill
      ...
      <div className="bonus-orbit orbit-one" />
      <div className="bonus-orbit orbit-two" />
    </div>
  </section>
  ...
  <section className="section">
    <div className="final">
      <div><div className="eyebrow">ENTER THE NETWORK</div><h2>Build today. Power tomorrow.</h2><p>Read the public information, then create your account and start building your virtual mining network.</p></div>
      <Link href="/auth/register" className="cta primary">Create Free Account <span aria-hidden="true">→</span></Link>
    </div>
  </section>
