...
export function DepositFlow() {
  ...
  async function submitDeposit() {
    if (!selected || !txHash.trim() || !cryptoAmount || !quote.usdPrice || !quote.fetchedAt) return;
    setSubmitting(true);
    setSubmitError('');
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : 'Unable to submit deposit');
    } finally {
      setSubmitting(false);
    }
  }
          ...
          <div className="deposit-payment-right">
            ...
            {!showTxStep ? (
              ...
              <div className="deposit-confirm-step">
                ...
                <button type="button" className="btn btn-success deposit-confirm-btn" onClick={submitDeposit} disabled={!txHash || submitting || submitted || !quoteFresh || !cryptoAmount}>
                  {submitting ? <><Loader2 size={16} className="spin" /> Submitting…</> : submitted ? <><Check size={16} /> Submitted — PENDING</> : txHash ? 'Submit for verification' : <><Loader2 size={16} /> Enter TX hash</>}
                </button>
                {submitError && <div className="deposit-warning"><ShieldAlert size={16} /><span>{submitError}</span></div>}
                {submitted && <div className="deposit-note">Deposit submitted successfully and is now PENDING. No 💎 is credited until the transaction is manually verified and approved.</div>}
                {!submitted && !submitError && <div className="deposit-note">Submitting creates a PENDING deposit only. It does not change the wallet balance from the browser.</div>}
              </div>
            )}
          </div>
        </div>
