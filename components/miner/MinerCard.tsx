...
export function MinerCard({ miner, onChanged }: MinerCardProps) {
  ...
  const atMax =
    miner.owned && miner.currentLevel >= miner.maxLevel;

  const normalizedImage =
    miner.image?.trim() || `/assets/miners/${miner.slug}.webp`;
  ...
  return (
    ...
    >
      <div className="miner-visual">
        <img
          ...
          onError={(event) => {
            ...
            const fallback = `/assets/miners/${miner.slug}.webp`;
      ...
      <div className="miner-copy">
        ...
        {message && (
          <div className="miner-action-message">{message}</div>
        )}
      </div>
    </article>
  );
