-- CP08 production migration: AdGem signed-postback reward/revenue ingestion.
-- This migration was applied to production under migration name adgem_offerwall_integration_v1.

insert into public.nextgen_offer_providers(provider_key,display_name,enabled)
values ('adgem','AdGem',false)
...
begin
  ...
  if v_user_reward_usd>p_payout_usd+0.00000001 then
    raise exception using errcode='P0001',message='ADGEM_REWARD_EXCEEDS_PAYOUT',detail=jsonb_build_object('payout_usd',p_payout_usd,'amount_diamond',p_amount_diamond,'diamond_per_usd',v_diamond_per_usd,'user_reward_usd',v_user_reward_usd)::text;
  end if;
  ...
  values (p_player_id,'adgem',round(p_amount_diamond,8),v_reward_ref,'posted',coalesce(p_payload,'{}'::jsonb)||jsonb_build_object('provider','adgem','request_id',trim(p_request_id),'conversion_id',trim(p_conversion_id),'campaign_id',trim(p_campaign_id),'payout_usd',round(p_payout_usd,8),'diamond_per_usd',v_diamond_per_usd,'user_reward_usd',v_user_reward_usd))
  ...
  values (v_event_date,'offerwall',v_source_ref,round(p_payout_usd,8),v_user_reward_usd,round(p_payout_usd-v_user_reward_usd,8),'settled',coalesce(p_payload,'{}'::jsonb)||jsonb_build_object('provider','adgem','request_id',trim(p_request_id),'conversion_id',trim(p_conversion_id),'campaign_id',trim(p_campaign_id),'offer_id',v_offer.id,'reward_ledger_id',v_reward_id,'offer_completion_id',v_completion_id),'adgem',now(),'AdGem signed payable conversion accepted as recoverable.')
