...
type BonusStatus = {
  ...
  bonus_miner?: { name?: string; hashrate?: number };
};
...
export default function Register() {
  ...
  useEffect(() => {
    ...
    supabase.rpc('nextgen_get_registration_bonus_status').then(({ data }) => {
      if (!mounted) return;
      const status = data as BonusStatus | null;
      if (typeof status?.remaining_slots === 'number') {
        setRemainingSlots(status.remaining_slots);
      }
    });
  ...
  return (
    <div className="auth-page">
      <div className="glass auth-card">
        ...
        <form className="form" onSubmit={submit}>
          <div className="field">
            ...
            <input className="input" autoComplete="username" value={username}
              onChange={(e) => setUsername(e.target.value)} placeholder="MinerX" required />
          </div>
          ...
          {message && <div className={success ? 'notice' : 'notice'}>{message}</div>}
        ...
        <p className="muted">
          Already registered? <Link href="/auth/login" style={{ color: 'var(--cyan)' }}>Login</Link>
        </p>
      </div>
    </div>
  );
