...
const FALLBACK_ASSETS = [
  "BCH", "BNB", "BTC", "DASH", "DGB", "DOGE", "DOL", "ETH",
  "FEY", "LTC", "SOL", "TRX", "USDC", "USDT", "ZEC",
];
...
const shortType = (value: string) =>
  value
    .replaceAll("_", " ")
    .toLowerCase()
    .replace(/(^|\s)\S/g, (c) => c.toUpperCase());

function iconTone(asset: string) {
...
export default function Wallet() {
  ...
  const loadWallet = useCallback(async (soft = false) => {
    ...
    try {
      ...
      const [wallet, crypto, supported, nets, tx, rateRows] = await Promise.all([
        sb.from("nextgen_wallets")
          .select("diamond_balance")
          .eq("user_id", auth.user.id)
          .maybeSingle(),
        sb.from("nextgen_crypto_balances")
          .select("asset,balance,reserved_balance")
          .eq("user_id", auth.user.id)
          .order("asset"),
        sb.from("nextgen_supported_crypto_assets")
          .select("asset")
          .eq("withdrawal_enabled", true)
          .order("asset"),
        sb.from("nextgen_crypto_networks")
          .select("asset,network")
          .eq("enabled", true)
          .order("asset")
          .order("network"),
        sb.from("nextgen_transactions")
          .select(
            "id,tx_type,asset,network,crypto_amount,usd_delta,diamond_delta,note,created_at"
          )
          .eq("user_id", auth.user.id)
          .order("created_at", { ascending: false })
          .limit(30),
        sb.from("nextgen_exchange_rate_history")
          .select("asset,rate_usd,observed_at")
      ...
      if (supported.data?.length) {
        setAssets(supported.data.map((x: any) => String(x.asset)));
      }
      ...
      for (const x of rateRows.data ?? []) {
        ...
        latest.push({
          asset,
          rate_usd: Number(x.rate_usd ?? 0),
          observed_at: String(x.observed_at),
        });
      }
      setRates(latest);
    } finally {
  ...
  const balanceMap = useMemo(
    () =>
      new Map(
        balances.map((b) => [
          b.asset.toUpperCase(),
          Math.max(0, b.balance - b.reserved_balance),
        ])
      ),
    [balances]
  );
  ...
  const totalUsd = useMemo(
    () =>
      balances.reduce((sum, b) => {
        ...
        const rate = rateMap.get(b.asset.toUpperCase()) ?? 0;
  ...
  const filteredAssets = useMemo(() => {
    ...
    return assets.filter((asset) => !clean || asset.includes(clean));
  }, [assets, query]);

  const netFor = useCallback(
    (asset: string) =>
      networks
        .filter((n) => n.asset.toUpperCase() === asset.toUpperCase())
        .map((n) => n.network),
    [networks]
  );

  const available = useCallback(
    (asset: string) => balanceMap.get(asset.toUpperCase()) ?? 0,
    [balanceMap]
  );

  useEffect(() => {
    const ns = netFor(withdrawAsset);
    if (ns.length && !ns.includes(withdrawNetwork)) {
      setWithdrawNetwork(ns[0]);
    } else if (!ns.length) {
      setWithdrawNetwork("");
    }
  }, [netFor, withdrawAsset, withdrawNetwork]);
  ...
  async function withdraw() {
    ...
    if (!Number.isFinite(n) || n <= 0) {
      setWithdrawMsg("Masukkan jumlah crypto yang valid.");
      return;
    }
    if (!withdrawAddress.trim()) {
      setWithdrawMsg("Masukkan alamat penerima.");
      return;
    }
    if (!withdrawNetwork) {
      setWithdrawMsg("Network belum tersedia untuk aset ini.");
      return;
    }
    if (n > available(withdrawAsset)) {
      setWithdrawMsg("Jumlah melebihi saldo tersedia.");
      return;
    }
    ...
    try {
      ...
      setWithdrawMsg(
        e instanceof Error ? e.message : "Withdrawal gagal diproses."
      );
    } finally {
  ...
  async function exchange() {
    ...
    if (!Number.isFinite(n) || n <= 0) {
      setExchangeMsg("Masukkan jumlah crypto yang valid.");
      return;
    }
    if (n > available(exchangeAsset)) {
      setExchangeMsg("Jumlah melebihi saldo tersedia.");
      return;
    }
    ...
    try {
      ...
      setExchangeOk(
        "Berhasil ditukar menjadi " +
          new Intl.NumberFormat("en-US").format(Number(d.diamondAmount ?? 0)) +
          " 💎."
      );
      setExchangeAmount("");
      await loadWallet(true);
      ...
      setExchangeMsg(
        e instanceof Error ? e.message : "Exchange gagal diproses."
      );
    } finally {
  ...
  if (loading) {
    return (
      <AppShell>
        <div className="wallet-simple-loading">
  ...
  return (
    <AppShell>
      <div className="wallet-simple-page">
        ...
        <div className="wallet-simple-tabs" role="tablist">
          <button
        ...
        {view === "summary" ? (
          <>
                ...
                <section className="wallet-coins-section">
                  ...
                  <div className="wallet-coin-list">
                    {filteredAssets.map((asset) => {
                      ...
                      const usdRate = rateMap.get(asset.toUpperCase()) ?? 0;
                      const usd = amount * usdRate;

                      return (
                        <article className="wallet-coin-card" key={asset}>
                          <div className={`wallet-coin-icon ${iconTone(asset)}`}>
                            {iconLetters[asset] ?? asset.slice(0, 2)}
                          </div>

                          <div className="wallet-coin-main">
                            <div className="wallet-coin-name">{asset}</div>
                            <div className="wallet-coin-amount">{fmtCrypto(amount)}</div>
                          </div>

                          <div className="wallet-coin-right">
                            <strong>{usdRate > 0 ? fmtUsd(usd) : "$0"}</strong>
                  ...
                  {!filteredAssets.length && (
                    <div className="wallet-empty-search">
                      <Sparkles size={18} />
            ...
            {action === "withdraw" && (
              <section className="wallet-action-panel">
                ...
                <div className="wallet-action-title">
                  <h2>Penarikan</h2>
                  <p>Kirim crypto dari saldo wallet Anda.</p>
                </div>
                ...
                <div className="wallet-simple-form">
                  ...
                  <div className="wallet-asset-strip">
                    {assets.map((asset) => (
                      ...
                      >
                        <span className={`mini-token ${iconTone(asset)}`}>
                          {iconLetters[asset] ?? asset.slice(0, 2)}
                        </span>
                        {asset}
                      </button>
                  ...
                  <label className="wallet-form-field">
                    ...
                    >
                      ...
                      {netFor(withdrawAsset).map((network) => (
                  ...
                  <label className="wallet-form-field">
                    <span>JUMLAH ({withdrawAsset})</span>
                    <div className="wallet-max-input">
                      <input
                      ...
                      <button
                        type="button"
                        onClick={() => setWithdrawAmount(String(available(withdrawAsset)))}
                      >
                        MAX
                      </button>
                    </div>
            ...
            {action === "exchange" && (
              <section className="wallet-action-panel">
                ...
                <div className="wallet-action-title">
                  <h2>Tukar ke 💎</h2>
                  <p>Crypto ditukar menjadi Diamond untuk ekonomi NextGenMiner.</p>
                </div>
                ...
                <div className="wallet-simple-form">
                  ...
                  <div className="wallet-asset-strip">
                    {assets.map((asset) => (
                      <button
                        key={asset}
                        type="button"
                        className={exchangeAsset === asset ? "selected" : ""}
                        onClick={() => setExchangeAsset(asset)}
                      >
                        <span className={`mini-token ${iconTone(asset)}`}>
                          {iconLetters[asset] ?? asset.slice(0, 2)}
                        </span>
                  ...
                  <div className="wallet-current-balance">
                    ...
                    <strong>{fmtCrypto(available(exchangeAsset))} {exchangeAsset}</strong>
                  </div>

                  <label className="wallet-form-field">
                    <span>JUMLAH ({exchangeAsset})</span>
                    <div className="wallet-max-input">
                      <input
                      ...
                      <button
                        type="button"
                        onClick={() => setExchangeAmount(String(available(exchangeAsset)))}
                      >
                        MAX
                      </button>
                    </div>
                  ...
                  <button
                    type="button"
                    className="wallet-wide-button"
                    disabled={exchangeBusy || !available(exchangeAsset)}
          ...
          <section className="wallet-history-simple">
            <div className="wallet-section-title">
              <div>
                <h2>Riwayat</h2>
                <p>Setoran, penarikan, dan tukar.</p>
              </div>
              <History size={20} />
            </div>
            ...
            {activity.length ? (
              <div className="wallet-history-list">
                {activity.map((tx) => {
                  const incoming =
                    Number(tx.crypto_amount) > 0 ||
                    Number(tx.usd_delta) > 0 ||
                    Number(tx.diamond_delta) > 0;

                  return (
                    <div className="wallet-history-row" key={tx.id}>
                      <div className={`wallet-history-icon ${incoming ? "positive" : ""}`}>
                        {tx.tx_type.toLowerCase().includes("withdraw") ? (
                      ...
                      <div className="wallet-history-copy">
                        ...
                        <small>
                          {tx.asset
                            ? `${tx.asset}${tx.network ? ` • ${tx.network}` : ""}`
                            : tx.note ?? "Aktivitas wallet"}
                        </small>
                      </div>
                      ...
                      <div className="wallet-history-value">
                        <b>
                          {tx.crypto_amount
                            ? `${fmtCrypto(Math.abs(tx.crypto_amount))} ${tx.asset ?? ""}`
                            : tx.diamond_delta
                              ? `💎 ${new Intl.NumberFormat("en-US").format(Math.abs(tx.diamond_delta))}`
                              : "Updated"}
                        </b>
              ...
              <div className="wallet-empty-history">
                ...
                <span>Aktivitas wallet akan muncul di sini.</span>
              </div>
            )}
          </section>
        )}

        <div className="wallet-simple-security">
          <ShieldCheck size={16} />
          <span>Saldo dan settlement tetap diverifikasi server-side.</span>
        </div>
      </div>
    </AppShell>
  );
