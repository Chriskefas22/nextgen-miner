    ...
    useCallback(
      async () => {
        ...
        try {
          ...
          const [
            ...
            await Promise.all([
              sb
                ...
                .select(
                  'id,miner_id,current_level,status,is_merged,deployment_state,recharge_expires_at',
                )
                .eq(
                  'user_id',
                  user.id,
              ...
              sb
                ...
                .select(
                  'id,slug,name,tier,base_hashrate,base_power_watts,image_path',
                )
                .eq(
                  'enabled',
                  true,
          ...
          for (
            const room of nextRooms
          ) {
            for (
              const slot of room.slots ??
              []
            ) {
              slotByMiner.set(
                Number(
          ...
          for (
            ...
            const key =
              `${slot.roomId}:${row.miner_id}:${row.current_level}`;
                ...
                const mergeReady =
                  Boolean(
                    slot &&
                      ...
                      (
                        pairs.get(
                          `${slot.roomId}:${row.miner_id}:${level}`,
                        ) ?? 0
                      ) >= 2 &&
                      level < 10,
                  );
        ...
        {loading ? (
          ...
          <section className="inventory-empty">
            ...
            <p>
              {items.length
                ? 'Try another filter.'
                : 'Visit the Shop to buy a miner. Purchased miners will appear here before deployment.'}
            </p>
              ...
              (item) => (
                ...
                >
                  <div className="inventory-art">
                    ...
                    <span
                      className={`inventory-state ${item.deployment_state}`}
                    >
                      {item.deployment_state ===
                      'deployed'
                        ? 'DEPLOYED'
                    ...
                    {item.mergeReady ? (
                      <span className="inventory-merge">
                        MERGE READY
                      </span>
                    ) : null}
                  </div>
                  ...
                  <div className="inventory-copy">
                    ...
                    'inventory' ? (
                      ...
                      /*
                       * Intentionally no Return/Open Room buttons here.
                       * Those controls now live in Rooms / Rack, keeping
                       * Inventory focused on un-deployed miners.
                       */
                      <div className="inventory-deployed-note">
                        Managed from Rooms / Rack.
                      </div>
                    )}
                  </div>
                </article>
