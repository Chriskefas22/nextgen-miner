...
import { ArrowLeft, Boxes, GitMerge, LockKeyhole, Sparkles, X } from 'lucide-react';
...
export default function RoomDetailPage() {
  ...
  const [busy, setBusy] = useState<'unlock' | 'merge' | null>(null);
  ...
  function selectMiner(slot: Slot) {
    ...
    setMessage('Select another matching miner at the same level.');
  }
  ...
  async function mergeSelected() {
    ...
    try {
      ...
      const payload = result.data as
        | { bonus_hashrate_percent?: number; to_level?: number }
        | null;

      setSelectedIds([]);
      setMessage(
        `Merge complete · Level ${payload?.to_level ?? 'next'} · Bonus +${num(
    ...
    return (
      <AppShell>
        <div className={styles.page}>
          <section className={styles.loadingPanel}>SYNCING ROOM…</section>
        </div>
      </AppShell>
    );
  }
      ...
      <AppShell>
        <div className={styles.page}>
          <section className={styles.lockPanel}>
            <div className={styles.lockIcon}><LockKeyhole size={26} /></div>
            <div className={styles.kicker}>ROOM {String(roomNumber).padStart(2, '0')}</div>
            <h1>Room {String(roomNumber).padStart(2, '0')}</h1>
            <p>Unlock this Room to create another independent 12-slot mining workspace.</p>
            {isNextLockedRoom ? (
              <>
  ...
  const slots = Array.from(
    { length: 12 },
    (_, index) => room.slots.find((slot) => slot.slot_index === index + 1) ?? null,
  );
      ...
      <div className={styles.page}>
        ...
        <section className={styles.roomHero}>
          ...
          <div className={styles.roomHeroBadge}>
            <Sparkles size={15} /> WORKSPACE ONLINE
          </div>
        </section>
        ...
        <section className={styles.roomControls}>
          ...
          {selectedIds.length === 2 ? (
            ...
            <div className={styles.manualHint}>
              Tap two matching miners to merge them manually.
            </div>
          )}
        </section>
            ...
            {slots.map((slot, index) =>
              slot ? (
                ...
                >
                  ...
                  <em>+{num(slot.bonus_hashrate_percent, 1)}%</em>
                </button>
              ) : (
                <div
                  key={`empty-${index}`}
